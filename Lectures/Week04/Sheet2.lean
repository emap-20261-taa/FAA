...
In Lean 4, an inductive type is a way to define a new type by specifying its
constructors, which describe all possible ways to create values of that type.

* An inductive type is defined by listing its constructors. Each constructor can
  take arguments, allowing the creation of more complex structures.
* Inductive types can be recursive, meaning a constructor can include the type
  itself as part of its arguments. This enables defining structures like lists
  or trees.
...
-- Here, If OptionNat is a set, then it contains {.none, .some 1, .some 2, .some
-- 3, ...}
...
#check (MyOption.none : MyOption ℚ)
...
-- Equivalently, it can be written this way in the functional form
-- (Haskell-like)
...
-- Define a type of binary tree it is a leaf or a node with two subtrees
