...
-- We have seen inductive types in the previous class. Today, we cover the
-- concept of inductive predicate.

-- Inductive predicate is a proposition that is defined inductively which Lean
-- supports.
...
theorem completeness_isEven (n : ℕ): evenFun n → IsEven n := by
  ...
  rw [evenFun] at h
  obtain ⟨r,hr⟩ := h
  induction' r  with m generalizing n
  . rw [hr]
    simp [IsEven.zero]
  . rw [hr]
    simp [Nat.mul_add]
...
-- The arguments for inductive predicates evolves throughout the induction. we
-- can apply `induction h` on h: Even n. However, we cannot apply `induction h`
-- on h: Even (n+1) because in Lean (n+1) is not a variable. Example

-- generalize, replace, rwa tactics
theorem aux (m : ℕ) (h : IsEven (m + 2)) : IsEven m := by
  apply soundness_isEven (m + 2) at h
  rw [evenFun] at h
  obtain ⟨r, hr⟩ := h
  apply completeness_isEven
  rw [evenFun]
  use (r - 1)
  omega
...
example (m n : ℕ) (h: m = 2 * n + 1) : ¬ IsEven m := by
  intro h₁
  induction' h₁ with x hx generalizing n
  . contradiction
  . specialize a_ih (n - 1)
    apply a_ih
    omega
